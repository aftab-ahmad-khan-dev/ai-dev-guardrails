...
How to wire lifecycle skills from this pack ([`skills/`](../skills/)) into **Cursor**. Pair with [`Rules.md`](../Rules.md) and [`SRS.md`](../SRS.md).
...
your-project/
├── .cursor/
│   ├── rules/                    # Short .mdc policies (yours + thin pointers)
│   │   └── agent-skills.mdc      # Optional: “use project skills” pointer
│   └── skills/                   # What Cursor Agent loads (gitignored locally)
│       ├── using-agent-skills/
│       ├── test-driven-development/
│       ├── code-review-and-quality/
│       └── …                     # Synced from this pack’s skills/ + your own
├── skills/                       # Vendored in ai-dev-guardrails (or submodule)
├── Rules.md                      # Guardrails standards
└── SRS.md                        # Project description + task board
```

**Source of truth for the agent:** `.cursor/skills/` (runtime) synced from this repo’s [`skills/`](../skills/).  
Also keep [`SRS.md`](../SRS.md) + applicable [`Rules.md`](../Rules.md) sections in context.
...
**From this pack** (or any checkout that contains `skills/`):
...
```bash
rsync -a --ignore-existing /path/to/ai-dev-guardrails/skills/ .cursor/skills/
```
