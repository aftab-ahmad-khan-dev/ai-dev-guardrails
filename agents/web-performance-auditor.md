...
| Live trace, LCP attribution, INP attribution, layout shift attribution | Chrome DevTools MCP server (`performance_*`, `lighthouse_audit`) | `chrome-devtools` MCP server configured in the harness (see `skills/verify/browser-testing-with-devtools`) |
...
9. Delegate granular optimization guidance and remediation steps to `skills/review/performance-optimization/SKILL.md` — keep this report at the audit level.
